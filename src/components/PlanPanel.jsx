...
이 주제에 관한 최신 학술 연구(논문), 교과서, 전문 서적의 지식을 동원하여, 입력 주제와 100% 일치하는 단 하나의 영상을 기획해주세요.

[주제 — 이 문장과 100% 일치하는 영상 1개만 기획]
${seed}
...
[이번 기획의 특수성 — 단일 마스터 콘텐츠]
- 입력된 주제 문장을 그대로 다루는 "마스터 영상" 1개만 만듭니다.
- 주제를 다른 각도로 쪼개거나 변형/확장하지 마세요. 입력 주제의 키워드·범위를 1mm도 벗어나지 않습니다.
- 영상 하단 CTA에서는 "짱샘의 책방(jjangsaem.com)"으로 더 깊은 자료를 안내할 예정이니, "영상 하나로 큰 그림 → 책방에서 심화"가 자연스럽게 이어지도록 설계하세요.

[목표]
- 시청자가 이 영상 1개로 해당 주제의 핵심 그림을 이해
- 더 깊이 알고 싶으면 짱샘의 책방을 찾도록 자연스럽게 유도

[요청 — 반드시 정확히 1개만 생성]
★ 롱폼 영상 (일반 8~10분) 1개 — 주제와 100% 일치하는 단일 마스터 콘텐츠
  - format 값은 반드시 "일반 8~10분"
  - 다른 영상(쇼츠/추가 롱폼) 절대 추가 금지. items 배열 길이는 반드시 1.

[focus 필드 작성 가이드]
- focus는 이 영상이 참고할 구체적 학술 범위(이론명, 연구 분야, 핵심 개념, 저자)를 명시합니다.
- 입력 주제 문장의 모든 핵심 키워드를 focus 안에 명시적으로 포함시키세요.
- 나중에 대본 작성 시 이 범위 밖의 내용은 사용하지 않습니다.

제목은 입력 주제와 의미가 100% 일치하면서도 유튜브에서 클릭을 유발하는 형태로 작성하세요.

JSON으로 출력:
{
  "items": [
    { "title": "영상 제목 — 입력 주제와 100% 일치", "format": "일반 8~10분", "focus": "참고할 학술 범위 — 이론명/저자/연구 분야 구체적으로 + 입력 주제의 모든 키워드 포함", "desc": "이 영상에서 다룰 핵심 내용 1줄 요약 — 입력 주제 그대로" }
  ]
}
JSON만 출력. items 배열 길이는 반드시 1.`;
    ...
    <div className="panel-card">
      ...
      {mode === 'topic' && (
        <div className="form-group" style={{ padding: '1.25rem', border: '2px solid var(--primary)', borderRadius: 'var(--radius-md)', backgroundColor: 'var(--secondary)' }}>
          ...
          <p style={{ fontSize: '0.875rem', marginBottom: '0.75rem', color: 'var(--text-muted)' }}>
            입력한 주제와 100% 일치하는 롱폼 영상 <strong>1개</strong>만 기획합니다 (마스터 콘텐츠).
          ...
          <div style={{ marginTop: '1rem' }}>
            {!hasSeries && (
              ...
              >
                {isGeneratingSeries ? (
                  <><Loader2 className="animate-spin" size={18} /> 주제 분석 및 마스터 영상 기획 중...</>
                ) : (
                  '주제 분석하여 마스터 영상 1개 기획하기'
                )}
              </button>
            )}
            {hasSeries && (
